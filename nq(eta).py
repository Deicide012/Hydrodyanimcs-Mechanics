...
def get_eta(nq_value, param_value):
    eta_at_param1 = cs1(np.log10(nq_value))
    eta_at_param2 = cs2(np.log10(nq_value))
    eta_interpolated = np.interp(param_value, [param1, param2], [eta_at_param1, eta_at_param2])
    return eta_interpolated
...
def get_nq(eta_value, param_value, initial_guess=2000):
    def equation(nq):
        return get_eta(nq, param_value) - eta_value
    
    nq_solution = fsolve(equation, initial_guess)[0]
    return nq_solution

nq_max = max(nq_test)
eta_max = get_eta(nq_max, param_intermediate)
print(f'Max n_q: {nq_max}, Corresponding η at param {param_intermediate}: {eta_max}')

get_eta(2500, 211)
get_nq(75, 211)
print(f'η at n_q=2500 and param=211: {get_eta(2500, 211)}')
print(f'n_q at η=75 and param=211: {get_nq(75, 211)}')